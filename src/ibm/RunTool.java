...
public class RunTool {
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			...
			} else if (args_0.equalsIgnoreCase("-meet")) {
	...
	private static void checkJavaVersion() {
		...
		if (version != null) {
			try {
				...
				float fver = Float.valueOf(ver).floatValue();
				if (fver < 1.5F) {
					System.out.println("Your Java Version is " + version);
					System.out
							.println("You need minimum Java 1.5 to run IBM Data Movement Tool");
					System.exit(-1);
				}
			} catch (Exception e) {
				System.out
						.println("Java version could not be determined. Proceeding with the tool launch ....");
			}
		}
	}
}